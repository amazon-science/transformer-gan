...
import sys
from helpers import truncated_normal_
    ...
    def init_params(self):
        for param in self.parameters():
            if param.requires_grad and len(param.shape) > 0:
                ...
                if self.cfg.DISCRIMINATOR.CNN.init == 'uniform':
                    torch.nn.init.uniform_(param, a=-0.05, b=0.05)
                elif self.cfg.DISCRIMINATOR.CNN.init== 'normal':
                    torch.nn.init.normal_(param, std=stddev)
                elif self.cfg.DISCRIMINATOR.CNN.init == 'truncated_normal':
...
class CNNClassifier(CNNDiscriminator):
    ...
    def forward(self, inp):
        ...
        # pools = [F.max_pool2d(con, (con.size(2), 1)).squeeze(2) for con in cons]  # [batch_size * num_filter *
        # num_rep]
